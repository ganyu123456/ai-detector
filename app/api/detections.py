...
@router.get("")
async def list_detections(stream_id: Optional[int] = None, db: AsyncSession = Depends(get_db)):
    q = select(DetectionConfig)
    if stream_id is not None:
        q = q.where(DetectionConfig.stream_id == stream_id)
...
async def create_detection(body: DetectionCreate, db: AsyncSession = Depends(get_db)):
    ...
    if body.enabled:
        ...
        asyncio.create_task(detection_manager.reload_stream(body.stream_id))
...
async def update_detection(
    ...
    from app.services.detection_service import detection_manager
    asyncio.create_task(detection_manager.reload_stream(cfg.stream_id))
...
async def delete_detection(detection_id: int, db: AsyncSession = Depends(get_db)):
    ...
    from app.services.detection_service import detection_manager
    asyncio.create_task(detection_manager.reload_stream(stream_id))
